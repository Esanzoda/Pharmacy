...
public interface IEmployeeRepository : IBaseRepository<Employee>
{
    ...
    Task<List<Employee>> GetEmployeesByAddressAsync(string address, int page, int pageSize);
...
public class EmployeeRepository : BaseRepository<Employee>, IEmployeeRepository
{
    ...
    public Task<List<Employee>> GetEmployeesByAddressAsync(string address, int page, int pageSize)
    {
        return DbSet
            .Where(x => x.Address.ToLower() == address.ToLower())
            .OrderBy(x => x.Id)
            .Skip((page - 1) * pageSize)
            .Take(pageSize)
            .ToListAsync();
